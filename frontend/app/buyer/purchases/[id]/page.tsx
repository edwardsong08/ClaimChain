...
import {
  downloadBuyerPurchasedPackageExport,
  getBuyerPurchasedPackageDetail,
} from "@/services/buyer";
...
export default function BuyerPurchasedPackageDetailPage() {
  ...
  const packageDetailQuery = useQuery({
    queryKey: ["buyer-purchased-package-detail", packageId, token],
    queryFn: () => {
      if (!token) {
      ...
      return getBuyerPurchasedPackageDetail(packageId, token);
    },
  ...
  const exportMutation = useMutation({
    mutationFn: async () => {
      ...
      const { blob, filename } = await downloadBuyerPurchasedPackageExport(packageId, token);
      const objectUrl = URL.createObjectURL(blob);
      try {
        const anchor = document.createElement("a");
        anchor.href = objectUrl;
        anchor.download = filename || `claimchain-package-${packageId}.json`;
        anchor.rel = "noopener";
  ...
  const shouldRedirectForApproval =
    packageDetailQuery.isError && isApprovalGateForbiddenError(packageDetailQuery.error);
  ...
  if (packageDetailQuery.isError) {
    return (
      <main className="min-h-screen flex items-center justify-center px-6 py-10">
        <p className="text-sm text-red-600">
          {getErrorMessage(packageDetailQuery.error, "Purchased package detail unavailable.")}
        </p>
      </main>
    );
  }
  ...
  const exportErrorMessage = exportMutation.isError
    ? getErrorMessage(exportMutation.error, "Unable to export package.")
    : null;
      ...
      <div className="mx-auto w-full max-w-5xl space-y-6">
        ...
        <section className="space-y-3 rounded-lg border p-5">
          ...
          <div className="grid gap-2 text-sm">
            ...
            <p>Status: {formatStatus(packageDetailQuery.data.status)}</p>
            <p>Total Claims: {packageDetailQuery.data.totalClaims ?? 0}</p>
            <p>Total Face Value: {formatCurrency(packageDetailQuery.data.totalFaceValue)}</p>
            <p>Price: {formatCurrency(packageDetailQuery.data.price)}</p>
            <p>Created: {formatDate(packageDetailQuery.data.createdAt)}</p>
            <p>Purchased: {formatDate(packageDetailQuery.data.purchasedAt)}</p>
          </div>
        </section>

        <section className="space-y-3 rounded-lg border p-5">
          <h2 className="text-xl font-semibold">Purchased Access</h2>
          <p className="text-sm text-gray-600">
            This package has been purchased and is available for export.
          </p>
          <button
            type="button"
          ...
          >
            {exportMutation.isPending ? "Preparing Download..." : "Download / Export Package"}
          </button>
          {exportErrorMessage && <p className="text-sm text-red-600">{exportErrorMessage}</p>}
        </section>
            ...
            <ul className="space-y-2">
              {packageDetailQuery.data.claims.map((claim, index) => (
                <li key={claimKey(claim, index)} className="rounded-md border p-3 text-sm">
                  ...
                  <p>
                    Debt Age (days):{" "}
                    {typeof claim.debtAgeDays === "number" ? claim.debtAgeDays : "N/A"}
                  </p>
